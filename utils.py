...
def deploy(env_name, agent, num_trials=5, fps=30):
    ...
    """
    print("\nDeploying DQN Agent (matplotlib)...")
    try:
    ...
    for trial in range(num_trials):
        ...
        while not terminal:
            # Pure exploitation: no epsilon exploration during deployment
            action = agent.choose_action(np.array(state), use_epsilon=False)
            ...
            ax.set_title(f"DQN | Trial {trial + 1} | Reward: {total_reward}")
            fig.canvas.draw()
