...
from utils import load_config, plot_results, deploy
...
def warmup(env, agent):
    ...
    Args:
        ...
        agent (DQNAgent): The DQN agent whose buffer will be filled.
...
def train(env, agent, num_steps, all_rewards, all_losses,
          ...
          is_new_run=True):
    """
    Executes the main step-based training loop for DQN (2015) with Experience Replay.
    ...
    while agent.total_steps < num_steps:
        ...
        if agent.total_steps - last_print_step >= print_every:
            ...
            print(
                ...
                f"Avg Reward (Last {moving_avg_window}): {avg_reward:.2f}"
            )
        ...
        if checkpoint_every > 0 and agent.total_steps - last_checkpoint_step >= checkpoint_every:
            ...
            plot_results(all_rewards, all_losses, save_path=plot_filepath, moving_avg_window=moving_avg_window)
...
def main():
    """
    Entry point for the DQN (2015) training and evaluation script.
    """
    print("--- Starting DQN (2015) with Experience Replay: Breakout ---")
    ...
    # Seeding both NumPy and PyTorch ensures that random weight initialization,
    # epsilon-greedy exploration, and replay buffer sampling are deterministic.
    seed = cfg["seed"]
    np.random.seed(seed)
    torch.manual_seed(seed)

    # --- 3. Set Device ---
    # Priority: Apple Silicon GPU (MPS) > NVIDIA GPU (CUDA) > CPU.
    ...
    results_dir = os.path.join(SCRIPT_DIR, cfg["paths"]["results_dir"])
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)

    model_filepath = os.path.join(results_dir, cfg["paths"]["model_filename"])
    history_filepath = os.path.join(results_dir, cfg["paths"]["history_filename"])
    plot_filepath = os.path.join(results_dir, cfg["paths"]["plot_filename"])
    ...
    agent = DQNAgent(
        ...
        epsilon_start=eps_cfg["start"],
        epsilon_min=eps_cfg["min"],
        epsilon_decay_steps=eps_cfg["decay_steps"],
        device=device,
        buffer_capacity=buf_cfg["capacity"],
        batch_size=buf_cfg["batch_size"],
        learning_starts=buf_cfg["learning_starts"],
        target_update_freq=tgt_cfg["update_freq"],
        loss_function=agent_cfg["loss_function"],
        optimizer=agent_cfg["optimizer"],
        grad_clip_norm=agent_cfg.get("grad_clip_norm", 1.0),
        clip_rewards=agent_cfg.get("clip_rewards", True)
    )
    ...
    if mode == 'new':
        ...
        try:
            ...
            plot_results(episode_rewards, episode_losses, save_path=plot_filepath, moving_avg_window=train_cfg["plot_window"])
